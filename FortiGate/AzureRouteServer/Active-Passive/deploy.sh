...
"
query="[?virtualMachine.name.starts_with(@, 'vm-fg100-fgt-')].{virtualMachine:virtualMachine.name, publicIP:virtualMachine.network.publicIpAddresses[0].ipAddress,privateIP:virtualMachine.network.privateIpAddresses[0]}"
az vm list-ip-addresses --query "$query" --output tsv
echo "
 IP Public Azure Load Balancer:"
publicIpIds=$(az network lb show -g "$rg" -n "lb-hub100-prod-eastus-01" --query "frontendIpConfigurations[].publicIpAddress.id" --out tsv)
